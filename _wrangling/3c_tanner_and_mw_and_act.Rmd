...
author: "Jackie"
date: "2/11/2020; revisting 7/5/20; revisiting 3/4/21; 6/15/21; 8/4/21"
output: 
  html_document:
    toc: true
    toc_float: true
...
### joinging with previously scored adolescent perceived stress
```{r, message=FALSE}
asqmwfp <- 
  "~/Box/Mooddata_Coordinating/1_Lab_Coordinating/Users/JackieSchwartz/Dissertation/2_study2/asqmw.csv"
asqmw <-
  read_csv(asqmwfp) %>%
  mutate(ELS_ID = factor(ELS_ID)) %>%
  as_tibble()
asqactfp <- 
  "~/Box/Mooddata_Coordinating/1_Lab_Coordinating/Users/JackieSchwartz/Dissertation/2_study2/asqact.csv"
asqact <-
  read_csv(asqactfp) %>%
  mutate(ELS_ID = factor(ELS_ID)) %>%
  as_tibble()
...
## removing those in early puberty
those in early puberty have a fundamentally different experience 
```{r}
table(dem_and_tanner_mw_act_comb_merge6$tanner_average_for_mw)
earlytanmw <- c(2, 2, 2.5, 2.5, 2.5)
earlytanmwout <-  which(dem_and_tanner_mw_act_comb_merge6$tanner_average_for_mw %in% c(earlytanmw))
earlytanmwout
dem_and_tanner_mw_act_comb_merge6[earlytanmwout, ]$ELS_ID
# 166 188 202 216 189


table(dem_and_tanner_mw_act_comb_merge6$tanner_average_for_act)
earlytanact <- c(1.5, 2, 2.5, 2.5, 2.5)
earlytanactout <-  which(dem_and_tanner_mw_act_comb_merge6$tanner_average_for_act %in% c(earlytanact))
earlytanactout
dem_and_tanner_mw_act_comb_merge6[earlytanactout, ]$ELS_ID
# 87  166 188 202 216

dem_and_tanner_mw_act_comb_merge7 <-
  dem_and_tanner_mw_act_comb_merge6 %>%
  filter(
    !ELS_ID == "166",
    !ELS_ID == "188",
...
## reading in compliance
```{r}
rr_df <- "~/Box/Mooddata_Coordinating/1_Lab_Coordinating/Users/JackieSchwartz/Dissertation/0_MW_Act_Demo_Descriptives/MW_daily_sleep_data_reduced.csv"
rr <-
  read_csv(rr_df) %>%
  dplyr::select(
    ELS_ID, sleep_resp_rate
  ) %>%
  mutate(ELS_ID = factor(ELS_ID))

dem_and_tanner_mw_act_comb_merge8 <-
  left_join(
    dem_and_tanner_mw_act_comb_merge7,
    rr,
    by = "ELS_ID"
...
desc_all_demo <-
  ...
  mutate(
    tannermatch = 
      ifelse(
        actigraphy_or_mw == "simultaneous ahc" | actigraphy_or_mw == "Both",
        tanner_average_for_mw == tanner_average_for_act,
        NA
      ),
    bmimatch = 
      ifelse(
    ...
    hhmatch = 
      ifelse(
        actigraphy_or_mw == "simultaneous ahc" | actigraphy_or_mw == "Both",
        Household_Income_MW == Household_Income_ACT,
        NA
      ),
    asqmatch =
      ifelse(
        actigraphy_or_mw == "simultaneous ahc" | actigraphy_or_mw == "Both",
        asq_total_mw == asq_total_act,
        NA
      )
  )
...
desc_all_demo_desc <-
  desc_all_demo %>%
  filter(actigraphy_or_mw == "simultaneous ahc") %>%
  summarize(
    n = n(),
    ...
    tannermid = sum(tanner_average_for_mw < 4),
    tannermidperc = (tannermid/n)*100,
    tannerlate= sum(tanner_average_for_mw >= 4 & tanner_average_for_mw < 5),
    tannerlateperc = (tannerlate/n)*100,
    tannerpost = sum(tanner_average_for_mw == 5),
    ...
    rr_min = min(sleep_resp_rate, na.rm = TRUE),
    rr_max = max(sleep_resp_rate, na.rm = TRUE),
    rr_sd = sd(sleep_resp_rate, na.rm = TRUE),
    rr_mean = mean(sleep_resp_rate, na.rm = TRUE)
    )
```
...
act_desc <-
  ...
  summarize(
    ...
    tannermid = sum(tanner_average_for_act < 4),
    tannermidperc = (tannermid/n)*100,
    tannerlate= sum(tanner_average_for_act >= 4 & tanner_average_for_act < 5),
    tannerlateperc = (tannerlate/n)*100,
    tannerpost = sum(tanner_average_for_act == 5),
    tannerpostperc = (tannerpost/n)*100,
...
write_csv(dem_and_tanner_mw_act_comb_merge8, "dem_and_tanner_mw_act_comb_merge.csv")
```

### for those with simulataneous components

```{r}
simdf <-
  dem_and_tanner_mw_act_comb_merge8 %>%
  filter(actigraphy_or_mw == "simultaneous ahc")
write_csv(simdf, "SimultaneousAHC/dem_and_tanner_mw_act_simultaneous.csv")
```
